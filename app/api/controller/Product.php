...
class Product extends Api
{
    ...
    public function index(Request $request)
    {
        ...
        if ($openid) {
            ...
            if ($r) {
                $user_id = $r[0]->user_id;
                // 根据用户id、产品id,获取收藏表信息
                $rr = $this->getModel('UserCollection')
                    ->where([
                                'user_id' => [
                                                '=',$user_id
                                ],'p_id' => [
                ...
                $time = date("Y-m-d");
                // 根据用户id,在足迹表里插入一条数据
                $rr_collection = $this->getModel('UserFootprint')
                    ->where([
                                'user_id' => [
                                                '=',$user_id
                                ],'p_id' => [
                ...
                if (empty($rr_collection)) {
                    $rrr = $this->getModel('UserFootprint')->insert([
                                                                        'user_id' => $user_id,'p_id' => $id,'add_time' => nowDate()
                    ]);
                }
            }
        }
        // 根据产品id,查询产品数据
        $res = $this->getModel('ProductList')
            ->alias('a')
            ->join('configure c', 'a.id=c.pid', 'LEFT')
            ->fetchWhere([
                            'a.id' => [
                                        '=',$id
                            ],'a.status' => [
        ...
        ], 'a.*,c.price,c.yprice,c.attribute,c.img');
        if (! $res) {
            echo json_encode(array(
                                    'status' => 0,'err' => '该商品已下架！'
            ));
            ...
            $img_arr = [];
            $r = $this->getModel('ProductImg')
                ->where([
                            'product_id' => [
                                                '=',$id
                            ]
            ])
                ->fetchAll('product_url,id');
            if ($r) {
            ...
            // 取数组最后一个元素 并查询分类名称
            $cid = end($typeArr);
            $r_p = $this->getModel('ProductClass')
                ->where([
                            'cid' => [
                                        '=',$cid
                            ]
            ...
            $r_freight = $this->getModel('Freight')
                ->where([
                            'id' => [
                                        '=',$freight_id
                            ]
            ...
            $r_c = $this->getModel('Comments')
                ->alias('a')
                ->join('user m', 'a.uid=m.user_id', 'LEFT')
                ->fetchWhere([
                                'a.pid' => [
                                                '=',$id
                                ]
            ], 'a.id,a.add_time,a.content,a.CommentType,a.size,m.user_name,m.headimgurl');
            $arr = [];
            if ($r_c) {
                foreach ($r_c as $key => $value) {
                    ...
                    $comments_id = $va['id'];
                    $comment_res = $this->getModel('CommentsImg')
                        ->where([
                                    'comments_id' => [
                                                        '=',$comments_id
                                    ]
                    ])
                        ->fetchAll('comments_url');
                    $va['images'] = '';
                    if ($comment_res) {
                    ...
                    // -------------2018-07-27 修改
                    $ad_res = $this->getModel('ReplyComments')
                        ->where([
                                    'cid' => [
                                                '=',$comments_id
                                    ],'uid' => [
            ...
            $commodityAttr = [];
            $r_size = $this->getModel('Configure')
                ->where([
                            'pid' => [
                                        '=',$id
                            ]
            ...
            if ($r_size) {
                ...
                foreach ($r_size as $key => $value) {
                    ...
                    $arrayName = [];
                  $attrList[$key] = array(
                                                    'attrType' => '1','id' => md5($key),'attr' => [],'all' => [],'index'=>$key
                            );
                ...
                foreach ($r_size as $key => $value) {
                    ...
                    $skuBeanList[$key] = array(
                        ...
                        $attrList[$key]['all'] = $all;
                        $attrList[$key]['attr'] = $attr;
                        $attrList[$key]['attrName']=$name;
            ...
            $share = array(
                        'friends' => true,'friend' => true
            );
            echo json_encode(array(
                                    'status' => 1,'pro' => $product,'qj_price' => $qj_price,'qj_yprice' => $qj_yprice,'attrList' => $attrList,'skuBeanList' => $skuBeanList,'collection_id' => $collection_id,'comments' => $arr,'type' => $type,'wx_id' => $wx_id,'share' => $share,'zhekou' => $zhekou
            ));
            exit();
        }
    }
    ...
    public function save_formid(Request $request)
    {
        ...
        if ($formid != 'the formId is a mock one' && $formid != '') {
            $addres = $this->getModel('UserFromid')->insert([
                                                                'open_id' => $uid,'fromid' => $formid,'lifetime' => $lifetime
            ]);
            echo json_encode(array(
                                    'status' => 1,'succ' => $addres
            ));
        }
    }
    ...
    public function add_cart(Request $request)
    {
        ...
        if (empty($Uid) || empty($Goods_id) || empty($Goods_id) || empty($size_id)) {
            echo json_encode(array(
                                    'status' => 0,'info' => '添加失败请重新提交!!'
            ));
        } else {
         $user_id=$this->getUserId($Uid);
            
            $res_k = $this->getModel('Configure')
                ->where([
                            'pid' => [
                                        '=',$Goods_id
                            ],'num' => [
            ...
            if ($num >= $Goods_num) {
                ...
                $res = $this->getModel('Cart')
                    ...
                    ->fetchAll('Goods_num,id');
                if ($res) {
                    // 根据点击的类型进行修改
                    if ($pro_type == 'buynow') {
                        $r_u = $this->getModel('Cart')->saveAll([
                                                                    'Goods_num' => $Goods_num
                        ], [
                                'Uid' => [
                                            '=',$Uid
                        ...
                        $r_u = $this->getModel('Cart')
                            ...
                            ->inc('Goods_num', $Goods_num)
                            ->update();
                    }
                    echo json_encode(array(
                                            'status' => 1,'cart_id' => $res['0']->id
                    ));
                } else {
                    $r = $this->getModel('Cart')->insert([
                                                            'user_id' => $user_id,'Uid' => $Uid,'Goods_id' => $Goods_id,'Goods_num' => $Goods_num,'Create_time' => nowDate(),'Size_id' => $size_id
                    ],1);
                    if ($r) {
                        echo json_encode(array(
                                                'status' => 1,'cart_id' => $r
    ...
    public function listdetail(Request $request)
    {
        ...
        $r = $this->getModel('ProductList')
            ...
            ->fetchOrder([
                            $select => $sort
        ], 'a.id,a.product_title,volume,c.price,c.yprice,c.img,a.s_type,c.id AS sizeid', "$start,$end");
        if ($r) {
            $product = [];
            foreach ($r as $k => $v) {
                $imgurl = $img . $v->img; /* end 保存 */
                $product[$k] = array(
                                    'id' => $v->id,'name' => $v->product_title,'price' => $v->yprice,'price_yh' => $v->price,'imgurl' => $imgurl,'size' => $v->sizeid,'volume' => $v->volume,'s_type' => $v->s_type
                );
            }
            echo json_encode(array(
                                    'status' => 1,'pro' => $product
            ));
            exit();
        } else {
            echo json_encode(array(
    ...
    public function get_more(Request $request)
    {
        ...
        $r = $this->getModel('ProductList')
            ...
            ->fetchOrder([
                            'a.sort' => 'asc'
        ], 'a.id,a.product_title,a.volume,c.price,c.yprice,c.img,c.id AS sizeid', "$start,$end");
        if ($r) {
            $product = [];
            foreach ($r as $k => $v) {
                $imgurl = $img . $v->img; /* end 保存 */
                $product[$k] = array(
                                    'id' => $v->id,'name' => $v->product_title,'price' => $v->yprice,'size' => $v->sizeid,'price_yh' => $v->price,'imgurl' => $imgurl,'volume' => $v->volume
                );
            }
            echo json_encode(array(
                                    'status' => 1,'pro' => $product
    ...
    public function freight($freight, $num, $address)
    {
        ...
        if ($r_1) {
            ...
            if (empty($address)) {
                ...
                $sheng = $address['sheng'];
                $r_2 = $this->getModel('AdminCgGroup')
                    ->where([
                                'GroupID' => [
                                                '=',$sheng
                                ]
    ...
    public function Settlement(Request $request)
    {
        ...
        $yunfei = 0;
        // 根据微信id,查询用户id
        $r_user = $this->getModel('User')
            ->where([
                        'wx_id' => [
                                        '=',$uid
                        ]
        ])
            ->fetchAll('user_id,money,consumer_money');
        if ($r_user) {
        ...
        if (! empty($r_a)) {
            ...
            if (! empty($r_e)) {
                ...
                $r_e = $this->getModel('UserAddress')
                    ...
                    ->fetchAll('*');
                $arr['adds'] = (array) $r_e['0']; // 收货地址
                $update_rs = $this->getModel('UserAddress')->saveAll([
                                                                        'is_default' => 1
                ], [
                        'id' => [
                                    '=',$aaaid
        ...
        foreach ($typeArr as $key => $value) {
            ...
            ], 'a.Goods_num,a.Goods_id,a.id,m.product_title,m.volume,c.price,c.attribute,c.img,c.yprice,m.freight,m.product_class');
            if ($r_c) {
                ...
                if (array_key_exists($Goods_id, $distributor_products)) { // 检查数组里是否有指定的键名或索引
                    ...
                    if ($grade_id) {
                        $r_grade = $this->getModel('DistributionGrade')
                            ->where([
                                        'id' => [
                                                    '=',$grade_id
                                        ]
        ...
        // 查询自动满减设置
        $r_subtraction = $this->getModel('Subtraction')
            ->where([
                        'id' => [
                                    '=','1'
                        ]
        ...
        if ($r_subtraction) {
            ...
            if ($r_subtraction[0]->status == 1) {
                ...
                if ($man_money <= $zong) { // 当商品总价满足 包邮限制
                    $r_address = $this->getModel('AdminCgGroup')
                        ->where([
                                    'GroupID' => [
                                                    '=',$address['sheng']
                                    ]
        ...
        if ($pstuat) {
            ...
            $time = date("Y-m-d H:i:s"); // 当前时间
                                         // 查询消费金参数
            $scoremsg = $this->getModel('Setscore')->fetchOrder([
                                                                    'lever' => 'asc'
            ], 'lever,ordernum,scorenum');
            if ($scoremsg) {
            ...
            if ($r) {
                foreach ($r as $k => $v) {
                    ...
                    if (empty($rr)) { // 没有数据,表示优惠券没绑定
                        $hid = $v->hid; // 活动id
                        $money = $v->money; // 优惠券金额
                        $rr1 = $this->getModel('CouponActivity')
                            ->where([
                                        'id' => [
                                                    '=',$hid
                                        ]
                        ...
                        if ($activity_type == 1) { // 当活动为注册类型
                            if ($money >= $order_zong) {
                                ...
                                $update_rs = $this->getModel('Coupon')->saveAll([
                                                                                    'type' => 0
                                ], [
                                        'id' => [
                                                    '=',$id
                            ...
                            if ($product_class_id == 0) { // 当没设置商品分类
                                ...
                                $rr_1 = $this->getModel('ProductList')
                                    ->where([
                                                'product_class' => [
                                                                        'like',"%$product_class_id%"
                                                ]
                                ])
                                    ->fetchAll('product_class');
                                if ($rr_1) {
    ...
    public function Shopping(Request $request)
    {
        ...
        ], 'a.*,c.price,c.attribute,c.img,c.num as pnum,m.product_title,c.id AS sizeid');
        if ($r_c) {
            foreach ($r_c as $key => $value) {
                ...
                $arr[$key] = array(
                                'id' => $value->id,'uid' => $uid,'pnum' => $value->pnum,'sizeid' => $value->sizeid,'pid' => $value->Goods_id,'size' => $size,'price' => $value->price,'num' => $value->Goods_num,'pro_name' => $value->product_title,'imgurl' => $imgurl
                );
            }
        }
    ...
    public function addFavorites($openid, $pid)
    {
        ...
        if ($r) {
            $user_id = $r[0]->user_id;
            // 根据用户id,产品id,查询收藏表
            $r = $this->getModel('UserCollection')
                ->where([
                            'user_id' => [
                                            '=',$user_id
                            ],'p_id' => [
            ...
            if (! $r) {
                // 在收藏表里添加一条数据
                $r = $this->getModel('UserCollection')->insert([
                                                                    'user_id' => $user_id,'p_id' => $pid,'add_time' => nowDate()
                ]);
            }
        }
    }
    ...
    public function wallet_pay(Request $request)
    {
        ...
        $r_user = $this->getModel('User')
            ...
            ->fetchAll('password,money,user_id');
        if ($r_user) {
            $user_money = $r_user['0']->money; // 用户余额
            $userid = $r_user['0']->user_id; // 用户id
            ...
            if ($user_money >= $total) {
                ...
                if ($total > 0) {
                    ...
                    $rr = $this->getModel('Record')->insert([
                                                                'user_id' => $userid,'money' => $total,'oldmoney' => $user_money,'event' => $event,'type' => 4
                    ]);
                }
                echo json_encode(array(
                                        'status' => 1,'succ' => '扣款成功!'
    ...
    public function payment(Request $request)
    {
        ...
        if ($type == 'wallet_Pay' && $user_money < $total) { // 当余额小于付款金额
            ...
            foreach ($typeArr as $key => $value) {
                ...
                ], 'a.Size_id,a.Goods_num,a.Goods_id,a.id,m.product_title,m.volume,m.freight,c.price,c.attribute,c.img,c.yprice,c.unit');
                if (! empty($r_c)) {
                    $product = (array) $r_c['0']; // 转数组
                    $product['photo_x'] = $img . $product['img']; /* 拼接图片链接 */
                    ...
                    if (empty($freight_id) || $freight_id == 0) { // 当运费id不存在 或者 为0 时
                        ...
                        // 根据运费id,查询运费信息
                        $r2 = $this->getModel('Freight')
                            ->where([
                                        'id' => [
                                                    '=',$freight_id
                                        ]
                        ])
                            ->fetchAll('type,freight');
                        if ($r2) {
                    ...
                    // 循环插入订单附表
                    $beres = $this->getModel('OrderDetails')->insert([
                                                                        'user_id' => $userid,'p_id' => $pid,'p_name' => $product_title,'p_price' => $price,'num' => $num,'unit' => $unit,'r_sNo' => $sNo,'add_time' => nowDate(),'r_status' => 0,'size' => $size,'sid' => $size_id,'freight' => $freight
                    ]);
                    if ($beres < 1) {
                        $M->rollback();
                        echo json_encode(array(
                                                'status' => 0,'err' => '下单失败,请稍后再试!'
                        ));
                        exit();
                    }
                    // 删除对应购物车内容
            ...
            // 在订单表里添加一条数据
            $r_o = $this->getModel('Order')->insert([
            ...
            if ($r_o > 0) {
                if ($allow) {
                    ...
                    $update_rs = $this->getModel('User')
                        ->where([
                                    'user_id' => [
                                                    '=',$userid
                                    ]
                    ])
                        ->dec('consumer_money', $allow)
                        ->update();
                    
                    $event = $userid . '抵用' . $allow . '元消费金';
                    // 类型 1:转入(收入) 2:提现 3:管理佣金 4:使用消费金 5收入消费金 6 系统扣款
                    $insert_rs = $this->getModel('DistributionRecord')->insert([
                                                                                    'user_id' => $userid,'from_id' => $userid,'money' => $allow,'sNo' => $sNo,'level' => 0,'event' => $event,'type' => 4,'add_date' => nowDate()
                    ]);
                }
                // 返回
                $M->commit();
                $arr = array(
                            'pay_type' => $type,'sNo' => $sNo,'coupon_money' => $z_price,'coupon_id' => $coupon_id,'order_id' => $r_o
                );
                echo json_encode(array(
                                        'status' => 1,'arr' => $arr
                ));
    ...
    public function up_order(Request $request)
    {
        ...
        if ($r_user) {
            ...
            if ($d_yuan) {
                ...
                // 写入日志
                $rr = $this->getModel('CombinedPay')->insert([
                                                                'weixin_pay' => $weixin_pay,'balance_pay' => $d_yuan,'total' => $coupon_money,'order_id' => $order_id,'add_time' => nowDate(),'user_id' => $user_id
                ]);
                // 根据修改支付方式
                $r_combined = $this->getModel('Order')->saveAll([
                                                                    'pay' => combined_Pay
                ], [
                        'sNo' => [
                                    '=',$order_id
                ...
                $rr = $this->getModel('Record')->insert([
                                                            'user_id' => $userid,'money' => $coupon_money,'oldmoney' => $d_yuan,'event' => $event,'type' => 4
                ]);
            }
            ...
            if ($coupon_money <= 0 && $allow > 0) {
                ...
                $r_u = $this->getModel('Order')->saveAll([
                                                            'status' => 1,'pay' => consumer_pay,'trade_no' => $trade_no
                ], [
                        'sNo' => [
                                    '=',$order_id
                        ],'user_id' => [
                ...
                $rpay = $pay ?: 0;
                $r_u = $this->getModel('Order')->saveAll([
                                                            'status' => 1,'pay' => $rpay,'trade_no' => $trade_no
                ], [
                        'sNo' => [
                                    '=',$order_id
                        ],'user_id' => [
            ...
            if ($allow && $coupon_money > 0) {
                ...
                $rr = $this->getModel('CombinedPay')->insert([
                                                                '$zpay' => $coupon_money,'consumer_pay' => $allow,'total' => $total,'order_id' => $order_id,'add_time' => nowDate(),'user_id' => $user_id
                ]);
                // 根据修改支付方式
                $r_combined = $this->getModel('Order')->saveAll([
                                                                    'pay' => combined_Pay
            ...
            if ($r_u) {
                ...
                echo json_encode(array(
                                        'status' => 1,'succ' => '操作成功!','sNo' => $order_id,'coupon_money' => $coupon_money,'id' => $id,'pname' => $pname,'time' => $time,'qu' => $ds
                ));
                exit();
            } else {
                echo json_encode(array(
    ...
    public function comment(Request $request)
    {
        ...
        if ($r_user) {
            if ($pid && $attribute_id) {
                $r_o = $this->getModel('OrderDetails')
                    ->alias('a')
                    ->join('configure m', 'a.sid=m.id', 'LEFT')
                    ->fetchWhere([
                                    'a.r_sNo' => [
                                                    '=',$order_id
                                    ],'a.p_id' => [
                ...
                ], 'a.p_id as commodityId,m.img,a.size,a.sid');
            } else {
                $r_o = $this->getModel('OrderDetails')
                    ->alias('a')
                    ->join('configure m', 'a.sid=m.id', 'LEFT')
    ...
    public function t_comment(Request $request)
    {
        ...
        if ($type == 'file') {
            ...
            $count = $this->getModel('CommentsImg')->getCount("comments_id ='" . $id . "'", 'id');
            $count > 3 && exit(json_encode(array(
                                                'status' => 1,'err' => '最多上传3张图片'
            )));
            // 查询配置表信息
            $file = $request->file('imgFile');
            $imgURL_name = $this->uploadImg($file);
            if ($imgURL_name !== false)
                $res = $this->getModel('CommentsImg')->insert([
                                                                    'comments_url' => $imgURL_name,'comments_id' => $id,'add_time' => nowDate()
                ]);
            else
                $res = false;
            if ($res) {
            ...
            foreach ($comments as $key => $value) {
                ...
                if ($content != '' || count($images) != 0) {
                    $r_c = $this->getModel('Comments')
                        ...
                        ->fetchAll('oid');
                    if (empty($r_c['0'])) {
                        $lcid = $this->getModel('Comments')->insert([
                                                                        'oid' => $oid,'uid' => $user_id,'pid' => $pid,'attribute_id' => $attribute_id,'size' => $size,'content' => $content,'CommentType' => $CommentType,'add_time' => nowDate()
                        ], 1);
                        ...
                        if ($lcid > 0) {
                            ...
                            if ($rr) {
                                ...
                                if ($arr[5] == count($rr)) {
                                    $update_rs = $this->getModel('Order')->saveAll([
                                                                                        'status' => 5
                                    ], [
                                            'sNo' => [
                                                        '=',$oid
    ...
    public function new_product(Request $request)
    {
        ...
        if ($r) {
            ...
            foreach ($r as $k => $v) {
                ...
                $product[$k] = array(
                                    'id' => $v->id,'name' => $v->product_title,'price' => $price,'price_yh' => $price_yh,'imgurl' => $imgurl,'volume' => $v->volume,'s_type' => $v->s_type
                );
            }
            echo json_encode(array(
                                    'status' => 1,'pro' => $product
    ...
    public function choujiangjiesuan(Request $request)
    {
        ...
        $product = array();
        $r_c = $this->getModel('Draw')
            ->alias('a')
            ->join('product_list b', 'a.draw_brandid=b.id', 'inner')
            ->fetchWhere([
                            'a.id' => [
                                        '=',$choujiangid
                            ]
    ...
    public function choujiangpayment(Request $request)
    {
        ...
        if (! empty($role) && $role != 'undefined') { // 通过分享ID查询该团成员总数与设定拼团人数
            $r04 = $this->getModel('Draw')
                ->where([
                            'id' => [
                                        '=',$choujiangid
                            ]
            ])
                ->fetchAll('num,spelling_number,collage_number');
            if ($r04) {
        ...
        if ($type == 'balance_Pay' && $user_money < $total) { // 当余额小于付款金额
            ...
            if (! empty($r_c)) {
                ...
                // 循环插入订单附表
                $r_d = $this->getModel('OrderDetails')->insert([
                                                                    'user_id' => $userid,'p_id' => $pid,'p_name' => $product_title,'p_price' => $price,'num' => $num,'unit' => '件','r_sNo' => $sNo,'add_time' => nowDate(),'r_status' => 0,'size' => $size,'sid' => $size_id
                ]);
            } else {
                echo json_encode(array(
                                        'status' => 0,'err' => '请勿重复下单！'
            ...
            // 插入抽奖与用户关联表
            $r_r = $this->getModel('DrawUser')->insert([
                                                            'draw_id' => $choujiangid,'user_id' => $userid,'time' => nowDate(),'role' => $role
            ], 1);
            // 在订单表里添加一条数据
            $r_o = $this->getModel('Order')->insert([
                                                        'user_id' => $userid,'name' => $name,'mobile' => $mobile,'num' => $z_num,'z_price' => $z_price,'sNo' => $sNo,'sheng' => $sheng,'shi' => $shi,'xian' => $xian,'address' => $address,'remark' => $remark,'pay' => $type,'add_time' => nowDate(),'status' => 0,'coupon_id' => 0,'allow' => 0,'drawid' => $r_r
            ],1);
            if ($role == 0) {
                // 把抽奖与用户关联的
                $r06 = $this->getModel('DrawUser')->saveAll([
                                                                'role' => $r_r
                ], [
                        'id' => [
                                    '=',$r_r
            ...
            if ($r_d > 0 && $r_o > 0) {
                // 返回
                $arr = array(
                            'pay_type' => $type,'sNo' => $sNo,'coupon_money' => $total,'coupon_id' => 0,'order_id' => $r_o,'type1' => 11
                );
                if (! empty($bb)) {
                    if ($bb >= $num1) {
                        echo json_encode(array(
                                                'status' => 1,'arr' => $arr,'err' => '该团已满，生成新团！'
                        ));
                        exit();
                    } else {
                        echo json_encode(array(
                                                'status' => 1,'arr' => $arr,'err' => ''
                        ));
                        exit();
                    }
                } else {
