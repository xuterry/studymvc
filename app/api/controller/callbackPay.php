                ...
                if ($money && $money > 0.1 && $user_id) {
                    ...
                    if ($beres < 1) {
                        ...
                        echo json_encode(array(
                                                'status' => 0,'err' => '参数错误 code:41','sql' => $sql
                        ));
                        exit();
                    }
                    ...
                    if ($beres < 1) {
                        $db->rollback();
                        echo json_encode(array(
                                                'status' => 0,'err' => '参数错误 code:42','sql' => $sqlldr
                        ));
                        exit();
                    }
                }
     ...
     //   $log_->log_result($log_name, "【业务出错】:\n" . $xml . "\n");
        ...
        if ($type == 'CZ') {
            ...
            if (! $czres) {
                ...
                $this->cz($openid, $cmoney, $trade_no);
            }
        } else if ($type == 'PT') {
            ...
            if ($data['pagefrom'] == 'kaituan') {
                ...
                $ptres = $this->creatgroup($data);
            } else {
                
                $ptres = $this->can_order($data);
            }
