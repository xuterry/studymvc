...
class Template
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $class         = false !== strpos($type, '\\') ? $type : 'core\\view\\think\\driver\\' . ucwords($type);
    ...
    public function fetch($template, $vars = [], $config = [])
    {
        ...
        if ($template) {
            $cacheFile = $this->config['cache_path'] .DS. $this->config['cache_prefix'] . md5($this->config['layout_name'] . $template) . '.' . ltrim($this->config['cache_suffix'], '.');
    ...
    private function compiler(&$content, $cacheFile)
    {
        ...
        // 添加安全代码及模板引用记录
        $content = '<?php if (!defined(\'CORE_PATH\')) exit(); /*' . serialize($this->includeFile) . '*/ ?>' . "\n" . $content;
        // 编译存储
    ...
    public function parseTagLib($tagLib, &$content, $hide = false)
    {
        if (false !== strpos($tagLib, '\\')) {
            ...
            $className = '\\core\\view\\think\\taglib\\' . ucwords($tagLib);
        }
        /** @var Taglib $tLib */
    ...
    private function parseTemplateFile($template)
    {
        if ('' == pathinfo($template, PATHINFO_EXTENSION)) {
            ...
            $module = isset($module) ? $module :Module::get_module();
            if ($this->config['view_base']) {            
                $path   = $this->config['view_base'] . ($module ? $module . DS : '');
            } else {
                $path = isset($module) ? APP_PATH . DS.$module . DS.'view' . basename($this->config['view_path']) . DS : $this->config['view_path'];
            }
