...
Page({
  ...
  createProductOrderByWX:function(e){
    ...
    if (parseInt(that.data.proattr.have) >= parseInt(that.data.groupres.groupnum)){
      ...
      if (address) {
        wx.request({
          url: app.d.ceshiUrl + '/groupbuy/isgrouppacked',
          method: 'post',
          data: {
            oid: that.oid, // 微信支付  
          },
  ...
  up_out_trade_no: function (status,trade_no) {
    var that = this;
    wx.request({
      url: app.d.ceshiUrl + '/groupbuy/up_out_trade_no',
      method: 'post',
      data: {
        uid: app.globalData.userInfo.openid,
        fromid: that.data.form_id,
  ...
  verification: function (trade_no) {
    var that = this;
    wx.request({
      url: app.d.ceshiUrl + '/groupbuy/verification',
      method: 'post',
      data: {
        trade_no: trade_no //微信交易单号
      },
