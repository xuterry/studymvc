...
Page({
  ...
  onShow: function () {
	...
	Settlement: function() {
		...
		if(type1 == 11) {
			...
			wx.request({
				url: app.d.ceshiUrl + '/product/choujiangjiesuan',
				method: 'post',
				data: {
			...
			wx.request({
        url: app.d.ceshiUrl + '/product/Settlement',
				method: 'post',
				data: {
					cart_id: that.data.cartId, // 购物车id
					uid: that.data.userId, // 微信id
	...
	my_coupon: function() {
		var that = this;
		wx.request({
			url: app.d.ceshiUrl + '/Coupon/my_coupon',
			method: 'post',
			data: {
	...
	getvou: function(id) {
		var that = this;
		wx.request({
			url: app.d.ceshiUrl + '/Coupon/getvou',
			method: 'post',
			data: {
				cart_id: that.data.cartId, // 购物车id
  ...
  switchChange: function (e) {
	...
	createProductOrder: function() {
      ...
      pages_sx: false
		...
		if(type1 == 11) { //抽奖
			...
			wx.request({
				url: app.d.ceshiUrl + '/product/choujiangpayment',
				method: 'post',
				data: {
					uid: that.data.userId, // 微信id
			...
			wx.request({
				url: app.d.ceshiUrl + '/product/payment',
				method: 'post',
				data: {
					uid: that.data.userId, // 微信id
					cart_id: that.data.cartId, // 购物车id
	...
	wallet_pay: function(order) {
		...
		*/
		...
		if(Number(user_money) >= Number(coupon_money)) {
			wx.request({
				url: app.d.ceshiUrl + '/product/wallet_pay',
				method: 'post',
				data: {
    ...
    if(that.data.trade_no){
      ...
      var trade_no = ''
		...
		wx.request({
			url: app.d.ceshiUrl + '/product/up_order',
			method: 'post',
			data: {
  ...
  get_plug: function (e) {
    var that = this;
    wx.request({
      url: app.d.ceshiUrl + '/app/get_plug',
      method: 'post',
      data: {
	...
	notice: function(order_id, order_sn, price, user_id, form_id, f_pname) {
		wx.request({
			url: app.d.ceshiUrl + '/getcode/Send_Prompt',
			method: 'post',
			data: {
  ...
  up_out_trade_no: function (order, out_trade_no){
    ...
    wx.request({
      url: app.d.ceshiUrl + '/order/up_out_trade_no',
      method: 'post',
      data: {
        coupon_id: order.coupon_id, // 优惠券id
        allow: that.data.allow, // 使用积分
